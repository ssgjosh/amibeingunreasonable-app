...
export default function Home() {
    ...
    const handleRestart = () => { setContext(''); setQuery(''); setResponses([]); setSummary(''); setParaphrase(''); setError(''); setSelectedPersona(null); setHasAnalyzed(false); setView('input'); setLoading(false); };
     ...
     // Handle Persona Selection with Delay/Animation Trigger
     const handleSelectPersona = (persona) => {
        if (persona === selectedPersona || isSwitchingPersona) return; // Prevent rapid clicks or re-selecting same

        setIsSwitchingPersona(true);
        if (detailViewRef.current) {
            detailViewRef.current.classList.remove('animate-fadeIn');
            void detailViewRef.current.offsetWidth; // Force reflow
        }

        setTimeout(() => {
            setSelectedPersona(persona);
            // Delay setting switching false slightly longer to ensure state update completes
            setTimeout(() => {
                setIsSwitchingPersona(false);
                // Add animation class after state is surely updated
                 requestAnimationFrame(() => {
                    if (detailViewRef.current) {
                        detailViewRef.current.classList.add('animate-fadeIn');
                    }
                 });
            }, 50); // Short delay after setting persona
        }, 150); // Initial delay for perceived loading
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-100 via-slate-50 to-blue-100 py-12 sm:py-16 px-4 sm:px-6 lg:px-8 font-sans antialiased">
            ...
            {/* Use conditional rendering carefully - ensure only ONE view block renders */}
            <div className={`max-w-5xl mx-auto bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-200/50 transition-opacity duration-300 ease-in-out ${view === 'loading' ? 'opacity-0 invisible' : 'opacity-100 visible'}`}>
                ...
                {view === 'input' && (
                    <div className="p-8 sm:p-10 lg:p-12 space-y-8 animate-fadeIn">
                         <div>
                             ...
                             <textarea id="context-input" className="w-full p-4 text-sm border border-gray-300 rounded-xl shadow-sm resize-vertical min-h-[200px] outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition duration-150 ease-in-out text-gray-800 placeholder-gray-500 bg-white/70" placeholder="Paste relevant chat logs or describe the events in detail..." value={context} onChange={(e) => setContext(e.target.value)} rows={10} />
                         ...
                         <div>
                             ...
                             <input id="query-input" type="text" className="w-full p-4 text-sm border border-gray-300 rounded-xl shadow-sm outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition duration-150 ease-in-out text-gray-800 placeholder-gray-500 bg-white/70" placeholder="e.g., Am I wrong? Is this unreasonable? What should I do?" value={query} onChange={(e) => setQuery(e.target.value)} />
                         </div>
                         {error && <div className="pt-2"><Alert type="error" message={error} /></div>}
                         <div className="text-center pt-6">
                             <button onClick={askAI} disabled={loading} className={`inline-flex items-center justify-center px-12 py-3.5 border border-transparent text-base font-semibold rounded-full shadow-lg text-white transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 focus:ring-offset-white transform hover:scale-105 ${ loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700'}`}>
                        ...
                        {responses.length > 0 && (
                            <div className="border-t border-gray-200 pt-10">
                                ...
                                <div className="flex justify-center flex-wrap gap-3 sm:gap-4 mb-10 border-b border-gray-300 pb-4">
                                     {responses.map((r) => ( r.response && !r.response.startsWith("[Error") && <button key={r.persona} onClick={() => handleSelectPersona(r.persona)} className={`px-5 py-2 text-sm font-medium rounded-full transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 whitespace-nowrap transform hover:scale-103 ${ selectedPersona === r.persona ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg ring-2 ring-offset-1 ring-cyan-300 scale-105' : 'text-slate-700 bg-white hover:bg-slate-200 border border-slate-300' }`}> {r.persona.split('(')[0].trim()} </button> ))}
                                 </div>
             ...
             <style jsx global>{`
                ...
                .animate-gradient-xy { background-size: 300% 300%; animation: gradient-xy 18s ease infinite; }
             `}</style>
        </div>
    );
}